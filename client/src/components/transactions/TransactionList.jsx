  ...
  return (
    <div className="animate-fade-in relative">
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr>
              ...
              <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-text-muted border-b border-glass-border whitespace-nowrap">Actions</th>
            </tr>
          </thead>
          <tbody>
            {transactions.map((tx, i) => (
              <tr key={tx._id} className="animate-fade-in-up hover:bg-bg-hover" style={{ animationDelay: `${i * 30}ms`, opacity: 0 }}>
                ...
                <td className="px-4 py-3 border-b border-glass-border text-[0.8125rem] align-middle">
                  <div className="flex items-center gap-1">
                    ...
                    <button className="p-2 rounded-[6px] text-text-muted transition-all duration-150 hover:text-expense hover:bg-expense-light" onClick={() => handleDelete(tx._id)} title="Delete">
                      <Trash2 size={15} />
                    </button>
                  </div>
                </td>
      ...
      <div className="hidden max-md:flex flex-col gap-2">
        {transactions.map((tx, i) => (
          <div key={tx._id} className="px-4 py-3 border-b border-glass-border animate-fade-in-up max-[360px]:px-3" style={{ animationDelay: `${i * 30}ms`, opacity: 0 }}>
            <div className="flex items-center gap-3">
              <CategoryIcon category={tx.category} size={16} />
