...
const CustomTooltip = ({ active, payload }) => {
  ...
  return (
    <div className="bg-bg-secondary border border-border rounded-md px-4 py-3 shadow-md">
      <p className="text-xs text-text-muted mb-1 font-semibold">{getCategoryLabel(data.category)}</p>
      <p className="text-[0.8125rem] font-semibold text-text-primary">{formatCurrency(data.amount)}</p>
      <p className="text-xs text-text-muted mt-1">{data.percentage}%</p>
    </div>
  );
};
  ...
  return (
    <Card className="animate-fade-in-up" padding="md">
      <div className="flex items-baseline justify-between mb-5">
        ...
        <span className="text-xs text-text-muted font-medium">By category</span>
      </div>
      <div className="flex items-center gap-6 max-sm:flex-col">
        ...
        <div className="flex-1 flex flex-col gap-2">
          {topCategories.map((item, index) => (
            <div key={item.category} className="flex items-center gap-2 text-[0.8125rem]">
