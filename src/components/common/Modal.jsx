  ...
  return (
    ...
    >
      <div
        ...
        ref={modalRef}
      >
        <div className="flex items-center justify-between px-6 py-5 border-b border-glass-border shrink-0 max-[360px]:px-4 max-[360px]:py-4">
          <h2 className="text-lg font-bold text-text-primary font-display">{title}</h2>
          {showClose && (
            <button
              className="text-text-muted p-2 rounded-full transition-all duration-150 hover:text-text-primary hover:bg-bg-hover"
              onClick={onClose}
